    ...
    <aside class="flex flex-col w-[220px] min-w-[220px] h-screen bg-white border-r border-[#e4e2db] px-3 py-5 z-10">
        ...
        <nav class="flex flex-col gap-0.5 flex-1">
            ...
            >
                ...
                >
                    ...
                    <span
                        v-if="item.badge"
                        class="ml-auto text-[10px] font-500 bg-[#1a7f72] text-white rounded-full px-1.5 min-w-[18px] h-[18px] inline-flex items-center justify-center leading-none"
                    >
                        {{ item.badge }}
                    </span>
                </button>
        ...
        <div class="mt-4 mx-1 rounded-lg bg-[#f5f4f1] border border-[#e4e2db] px-3 py-2.5">
            <div class="flex items-center justify-between mb-1.5">
                ...
                <span class="flex items-center gap-1 text-[11px] font-500"
                    :class="connected ? 'text-[#22c55e]' : 'text-[#a09f99]'">
                    <span class="w-1.5 h-1.5 rounded-full"
                        :class="connected ? 'bg-[#22c55e] animate-pulse' : 'bg-[#e4e2db]'"></span>
                    {{ connected ? 'Live' : 'Offline' }}
                </span>
            </div>
            <div class="flex items-center justify-between text-[11px] text-[#a09f99]">
                <span>Nodes online</span>
                <span class="font-mono font-500 text-[#1c1c1a]">
                    {{ store.onlineCount }} / {{ store.nodes.length }}
                </span>
            </div>
            <div class="mt-2 h-1 rounded-full bg-[#e4e2db] overflow-hidden">
                <div
                    class="h-full rounded-full bg-[#1a7f72] transition-all duration-500"
                    :style="{ width: store.nodes.length
                        ? (store.onlineCount / store.nodes.length * 100) + '%'
                        : '0%' }"
                ></div>
            </div>
        </div>
...
const connected = computed(() => store.error === null)
...
const IconSettings = () => h('svg', { viewBox: '0 0 16 16', fill: 'none', stroke: 'currentColor', 'stroke-width': '1.5', 'stroke-linecap': 'round', 'stroke-linejoin': 'round' }, [
    h('circle', { cx: '8', cy: '8', r: '2.5' }),
    h('path', { d: 'M8 1.5V3M8 13v1.5M1.5 8H3M13 8h1.5M3.4 3.4l1.1 1.1M11.5 11.5l1.1 1.1M3.4 12.6l1.1-1.1M11.5 4.5l1.1-1.1' }),
])
